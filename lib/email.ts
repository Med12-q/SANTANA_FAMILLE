...
function baseHtml(content: string) {
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"/><title>SANTANA FAMILLE</title></head>
...
export async function sendCandidateConfirmationEmail(to: string, fullName: string, pseudo: string) {
  ...
  const content = `
    ...
    <p style="font-size:13px;color:rgba(255,255,255,0.5);margin:0 0 20px;line-height:1.6;">Bonjour <strong style="color:#fff;">${fullName}</strong>, votre candidature sous le pseudo <strong style="color:#dc2626;">${pseudo}</strong> a bien été reçue.</p>
    <div style="background:rgba(37,211,102,0.08);border:1px solid rgba(37,211,102,0.2);border-radius:8px;padding:20px;margin:20px 0;">
      ...
      <p style="margin:0 0 14px;font-size:12px;color:rgba(255,255,255,0.5);line-height:1.6;">Rejoignez le groupe officiel pour les tests d'évaluation.</p>
      <a href="${WA_GROUP}" style="display:inline-block;background:#25D366;color:#fff;padding:10px 24px;border-radius:6px;text-decoration:none;font-size:12px;font-weight:700;">REJOINDRE LE GROUPE</a>
    </div>
    <p style="font-size:11px;color:rgba(255,255,255,0.25);font-style:italic;margin:16px 0 0;">"L'élite ne se rejoint pas. Elle se mérite."</p>
  `
  try {
    await getResend().emails.send({ from: FROM, to, subject: '✅ Candidature reçue — SANTANA FAMILLE', html: baseHtml(content) })
...
export async function sendAdminNotificationEmail(candidateName: string, pseudo: string, email: string, whatsapp: string, motivation: string) {
  ...
  `
  try {
    await getResend().emails.send({ from: FROM, to: NOTIFY_TO, subject: `🔔 Nouvelle candidature — ${candidateName} (${pseudo})`, html: baseHtml(content) })
